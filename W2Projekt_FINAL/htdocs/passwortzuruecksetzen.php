...
if(!isset($_GET['userid']) || !isset($_GET['code'])) {
	error("Leider wurde beim Aufruf dieser Website kein Code zum Zurücksetzen Ihres Passworts übermittelt");
}
...
if($user['passwortcode_time'] === null || strtotime($user['passwortcode_time']) < (time()-24*3600) ) {
	error("Ihr Code ist leider abgelaufen. Bitte benutzen Sie die Passwort vergessen Funktion erneut.");
}
 
 
//Überprüfe den Passwortcode
if(sha1($code) != $user['passwortcode']) {
	error("Der übergebene Code war ungültig. Stellen Sie sicher, dass Sie den genauen Link in der URL aufgerufen haben. Sollten Sie mehrmals die Passwort-vergessen Funktion genutzt haben, rufen Sie den Link in der neuesten E-Mail auf.");
}
